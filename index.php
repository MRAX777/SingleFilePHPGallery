...
option("SHOW_IMAGE_EXT", true);
option("IMAGE_SORT_REVERSE", false);
option("IMAGE_SORT_BY_TIME", true);
option("IMAGE_EXIF_TIME", true);
option("ROTATE_IMAGES", true);
option("IMAGE_JPEG_QUALITY", 99);
...
option("SHOW_INFO_BY_DEFAULT", true);
option("ROUND_CORNERS", 3);

option("ZIP_ENABLE", true);
...
option("THUMB_MAX_WIDTH", 175);
option("THUMB_MAX_HEIGHT", 175);
option("THUMB_SQUARE", true);
option("THUMB_ENLARGE", true);
option("THUMB_JPEG_QUALITY", 90);
...
option("THUMB_BOX_EXTRA_HEIGHT", 28);
option("THUMB_CHARS_MAX", 120);
...
option(
    "TEXT_PAYPAL_OUT_BACK",
    "Item is no longer available. Click back and refresh the page to update status."
);
option(
    "TEXT_PAYPAL_THANKS",
    "Thank you for your purchase. The item will remain listed as available until seller have verified the purchase. Click here to return to the gallery: "
);
...
option("TEXT_GALLERY_NAME", "Model Me! xXx");
option("TEXT_BANNER", "");
...
option(
    "TEXT_FIRST_VIEW",
    "This is first view of this image. Refresh page to get information."
);
...
option(
    "TEXT_REMOVE_THUMB",
    "No images selected. Will remove current directory thumbnail and select new thumbnail."
);
option("TEXT_SETTING_THUMB", "Setting directory thumbnail to: ");
option(
    "TEXT_WRONG_FILETYPE",
    "Wrong filetype selected. Filetype must be same as set in DIR_THUMB_FILE: "
);
...
function sfpg_url_decode($string)
{
    ...
    if (
        is_array($get) and
        count($get) == 3 and
        hash("sha256", $get[0] . "*" . $get[1] . "*" . SECURITY_PHRASE) ===
            $get[2] and
        strpos(GALLERY_ROOT . $get[0] . $get[1], "/../") === false and
        strpos($get[0] . $get[1], "\\") === false
    ) {
        return [$get[0], $get[1]];
    } else {
        return false;
...
function sfpg_mpo_image($file)
{
    ...
    if (count($imgOffset) > 2) {
        $img_left = imagecreatefromstring(
            substr($mpo, $imgOffset[0], $imgOffset[1] - $imgOffset[0])
        );
        $img_right = imagecreatefromstring(
            substr($mpo, $imgOffset[1], $imgOffset[2] - $imgOffset[1])
        );
        list($mpo_stereo_width, $mpo_stereo_height) = sfpg_aspect_resize(
            imagesx($img_left),
            imagesy($img_left),
            MPO_STEREO_MAX_WIDTH,
            MPO_STEREO_MAX_HEIGHT,
            true
        );
        list($mpo_full_width, $mpo_full_height) = sfpg_aspect_resize(
            imagesx($img_left),
            imagesy($img_left),
            MPO_FULL_MAX_WIDTH,
            MPO_FULL_MAX_HEIGHT,
        ...
        if (MPO_STEREO_IMAGE) {
            ...
            $new_img_height +=
                $stereo_dot_space +
                $mpo_stereo_height +
                (MPO_FULL_IMAGE ? MPO_SPACING : 0);
            $full_offset_y =
                $mpo_stereo_height + MPO_SPACING + $stereo_dot_space;
        }
        ...
        if (MPO_FULL_IMAGE) {
            if ($mpo_full_width > $new_img_width) {
                ...
                $stereo_align =
                    (int) (($mpo_full_width -
                        ($mpo_stereo_width * 2 + MPO_SPACING)) /
                        2);
                $full_offset_x = 0;
            }
        ...
        if (MPO_FULL_IMAGE) {
            if (MPO_FULL_ANAGLYPH) {
                $anaglyph_image = imagecreatetruecolor(
                    $mpo_full_width,
                    $mpo_full_height
                );
                ...
                for ($y = 0; $y < $mpo_full_height; $y++) {
                    for ($x = 0; $x < $mpo_full_width; $x++) {
                        ...
                        $r =
                            (int) (($left_color >> 16) & 255) * 0.299 +
                            (($left_color >> 8) & 255) * 0.587 +
                            ($left_color & 255) * 0.114;
                        if ($r > 255) {
                            $r = 255;
                        }
                        ...
                        imagesetpixel(
                            ...
                            $x,
                            $y,
                            imagecolorallocate($anaglyph_image, $r, $g, $b)
                        );
                    }
                }
                imagecopyresampled(
        ...
        if (MPO_STEREO_IMAGE) {
            ...
            imagecopyresampled(
                $new_image,
                $img_right,
                $stereo_align + $mpo_stereo_width + MPO_SPACING,
                $stereo_dot_space,
                0,
                0,
                $mpo_stereo_width,
            ...
            imagefilledrectangle(
                $new_image,
                $stereo_align + (int) ($mpo_stereo_width / 2) - 3,
                MPO_SPACING - 3,
                $stereo_align + (int) ($mpo_stereo_width / 2) + 3,
                MPO_SPACING + 3,
                $white
        ...
        list($mpo_width, $mpo_height) = sfpg_aspect_resize(
            imagesx($image),
            imagesy($image),
            MPO_FULL_MAX_WIDTH,
            MPO_FULL_MAX_HEIGHT,
...
function sfpg_get_dir($dir, $for_dir_info = false, $for_zip = false)
{
    ...
    if ($directory_handle != false) {
        while (($var = readdir($directory_handle)) !== false) {
            if (is_dir(GALLERY_ROOT . $dir . $var)) {
                if (
                    $var != "." and
                    $var != ".." and
                    !in_array(strtolower($var), DIR_EXCLUDE) and
                    !@preg_match(DIR_EXCLUDE_REGEX, $var)
                ) {
                    $dirs[] = $var;
                    if (DIR_SORT_BY_TIME) {
                        $dirs_time[] = filemtime(
                            GALLERY_ROOT . $dir . $var . "/."
                        );
                    }
                }
            } elseif (sfpg_image_type($var)) {
                if (
                    $var != DIR_THUMB_FILE and
                    !@preg_match(IMAGE_EXCLUDE_REGEX, $var)
                ) {
                    if (
                        DELETE_IMAGE_DAYS and
                        filemtime(GALLERY_ROOT . $dir . $var) <
                            time() - DELETE_IMAGE_DAYS * 86400
                    ) {
                        ...
                        if (IMAGE_SORT_BY_TIME) {
                            if (IMAGE_EXIF_TIME) {
                                if (
                                    $img_info = @file_get_contents(
                                        DATA_ROOT . "info/" . $dir . $var
                                    )
                                ) {
                ...
                if (
                    !in_array(strtolower($var), FILE_EXCLUDE) and
                    !in_array(sfpg_ext($var), FILE_EXT_EXCLUDE) and
                    !@preg_match(FILE_EXCLUDE_REGEX, $var)
                ) {
                    $files[] = $var;
            ...
            if (
                sfpg_ext($var) == DESC_EXT or
                sfpg_ext($var) == PAYPAL_EXTENSION
            ) {
                $misc[] = $var;
            }
        }
        ...
        if (SHOW_FILES and !($for_zip and ZIP_FILES and ZIP_FILE_THUMBS)) {
            // removes thumbnail images from being listed as images. Unless needed for zip.
...
{
    list($dirs, $images, $files, $misc) = sfpg_get_dir(
        $dir_to_zip,
        false,
        true
    );
    ...
    if (ZIP_SUB_GALLERIES) {
        foreach ($dirs as $dir) {
            $zip_list = array_merge(
                $zip_list,
                get_files_for_zip($dir_to_zip . $dir . "/")
            );
        }
    }
    return $zip_list;
...
function sfpg_browse_dirs()
{
    echo '<!DOCTYPE html><html lang="' .
        ...
        '"><head><meta charset="' .
        CHARSET .
        '"></head><body onload="parent.sendData(document.getElementById(\'dataContainer\').innerHTML)"><div id="dataContainer"><br>' .
        "&nbsp;" .
        TEXT_MOVE_TO .
        ": <b>" .
        TEXT_HOME .
        ...
        GALLERY .
        "</b><br><br>" .
        '<span class="sfpg_button" onclick="admMovePost(\'' .
        sfpg_url_string(GALLERY, "") .
        '\',\'' .
        TEXT_HOME .
        "/" .
        GALLERY .
        '\')">OK</span>' .
        '<span class="sfpg_button" onclick="admMovePost(false,false)">' .
        TEXT_CANCEL .
        "</span><br><br>" .
        '<span class="sfpg_button" onclick="admMove(\'' .
        sfpg_url_string() .
        '\')">' .
        TEXT_HOME .
        "</span>";
    ...
    foreach ($dirs as $dir) {
        if ($dir) {
            $path .= $dir . "/";
            echo '<div class="in"><span class="sfpg_button" onclick="admMove(\'' .
                sfpg_url_string($path, "") .
                '\')">' .
                $dir .
                "</span>";
            $postDiv .= "</div>";
    ...
    if ($items !== false) {
        ...
        foreach ($items as $var) {
            if (
                is_dir(GALLERY_ROOT . GALLERY . $var) and
                $var != "." and
                $var != ".."
            ) {
                echo '<span class="sfpg_button" onclick="admMove(\'' .
                    sfpg_url_string(GALLERY . $var . "/", "") .
                    '\')">' .
                    $var .
                    "</span><br>";
            }
...
function sfpg_div_num($input)
{
    $tmp = explode("/", $input);
    if (count($tmp) == 2) {
        // if input contains one /, like: "1/250"
        if (is_numeric($tmp[0]) and is_numeric($tmp[1]) and $tmp[1] != 0) {
            // if both sides are numbers and $tmp[1] is not 0.
            return $tmp[0] / $tmp[1];
        }
    }
    if (is_numeric($input)) {
...
function sfpg_image($image_dir, $image_file, $func, $download = false)
{
    ...
    if ($func == "thumb") {
        if (file_exists($image_path_file)) {
            ...
            $image_changed = false;
            if (
                (MPO_FULL_IMAGE or MPO_STEREO_IMAGE) and
                sfpg_ext($image_file) == ".mpo"
            ) {
            ...
            if ($func == "thumb" and $image_dir != ICONS_DIR) {
                ...
                if (function_exists("exif_read_data")) {
                    if (SHOW_EXIF_INFO) {
                        $exif_data = @exif_read_data(
                            GALLERY_ROOT . $image_dir . $image_file,
                            "IFD0"
                        );
                        if ($exif_data !== false) {
                            if (isset($exif_data["DateTimeOriginal"])) {
                                ...
                                if (count($exif_time) == 6) {
                                    $image_info["exifDate"] = block_html(
                                        mktime(
                                            (int) $exif_time[3],
                                            (int) $exif_time[4],
                                            (int) $exif_time[5],
                            ...
                            if (isset($exif_data["Model"])) {
                                $image_info["exifModel"] = block_html(
                                    $exif_data["Model"]
                                );
                            }
                            if (isset($exif_data["ISOSpeedRatings"])) {
                                $image_info["exifISOSpeedRatings"] = block_html(
                                    $exif_data["ISOSpeedRatings"]
                                );
                            }
                            if (isset($exif_data["ExposureTime"])) {
                                $exp_time = round(
                                    sfpg_div_num($exif_data["ExposureTime"]),
                                    2
                                );
                                if ($exp_time > 0.25) {
                                    $image_info[
                                        "exifExposureTime"
                                    ] = block_html($exp_time . "s");
                                } else {
                                    $image_info[
                                        "exifExposureTime"
                                    ] = block_html(
                                        $exif_data["ExposureTime"] . "s"
                                    );
                                }
                            }
                            if (isset($exif_data["FNumber"])) {
                                $image_info["exifFNumber"] = block_html(
                                    "f" .
                                        number_format(
                                            round(
                                                sfpg_div_num(
                            ...
                            if (isset($exif_data["FocalLength"])) {
                                $image_info["exifFocalLength"] = block_html(
                                    number_format(
                                        round(
                                            sfpg_div_num(
                            ...
                            if (isset($exif_data["Flash"])) {
                                $image_info["exifFlash"] = block_html(
                                    $exif_data["Flash"] & 1 ? TEXT_YES : TEXT_NO
                                );
                            }
                            if (
                                isset($exif_data["GPSLatitude"]) and
                                isset($exif_data["GPSLongitude"])
                            ) {
                                $lat = $exif_data["GPSLatitude"];
                                $lng = $exif_data["GPSLongitude"];
                                $image_info["exifGPSLatitude"] =
                                    block_html(sfpg_div_num($lat[0])) .
                                    "&deg;" .
                                    block_html(
                                        sfpg_div_num($lat[1]) .
                                            '\'' .
                                ...
                                $image_info["exifGPSLongitude"] =
                                    block_html(sfpg_div_num($lng[0])) .
                                    "&deg;" .
                                    block_html(
                                        sfpg_div_num($lng[1]) .
                                            '\'' .
                                ...
                                $image_info["exifGPSLatitudeDec"] = block_html(
                                    round(
                                        (sfpg_div_num($lat[0]) +
                                            sfpg_div_num($lat[1]) / 60 +
                                            sfpg_div_num($lat[2]) / 3600) *
                                            (@$exif_data["GPSLatitudeRef"] ==
                                            "N"
                                                ? 1
                                                : -1),
                                ...
                                ); // limited to 12 decimals
                                $image_info["exifGPSLongitudeDec"] = block_html(
                                    round(
                                        (sfpg_div_num($lng[0]) +
                                            sfpg_div_num($lng[1]) / 60 +
                                            sfpg_div_num($lng[2]) / 3600) *
                                            (@$exif_data["GPSLongitudeRef"] ==
                                            "W"
                                                ? -1
                                                : 1),
                ...
                if (SHOW_IPTC_INFO) {
                    $only_used_for_iptc = getimagesize(
                        GALLERY_ROOT . $image_dir . $image_file,
                        $info
                    );
                    if (isset($info["APP13"])) {
                ...
                if (PNG_TEXT_CHUNKS and sfpg_ext($image_file) == ".png") {
                    if (
                        ...
                        if ("\x89PNG\x0d\x0a\x1a\x0a" === fread($png_fp, 8)) {
                            // first 8 bytes in a PNG must be like this
                            $png_text_chunks = [];
                            while (!feof($png_fp)) {
                                $chunk_info = @unpack(
                                    "Nlength/a4type",
                                ...
                                if ($chunk_info["type"] == "tEXt") {
                                    ...
                                    if (
                                        ...
                                        list($keyword, $value) = explode(
                                            "\0",
                                            $chunk_data
                                        );
                                        if (
                                            PNG_TEXT_CHUNKS === true or
                                            is_array(PNG_TEXT_CHUNKS) and
                                                in_array(
                                                    strtolower($keyword),
                                                    PNG_TEXT_CHUNKS
                                                )
                ...
                if (WATERMARK) {
                    ...
                    if (file_exists($wm_file)) {
                        if (
                            $watermark = imagecreatefromstring(
                                file_get_contents($wm_file)
                            )
                        ) {
                            $image_width = imagesx($image);
                            $image_height = imagesy($image);
                            ...
                            if (WATERMARK_FRACTION) {
                                if ($image_width < $image_height) {
                                    $ww_new = round(
                                        $image_width * WATERMARK_FRACTION
                                    );
                                } else {
                                    $ww_new = round(
                                        $image_height * WATERMARK_FRACTION
                                    );
                                }
                ...
                if ($image_changed) {
                    ...
                    if ($image_type == "webp") {
                        imagewebp($image, $new_full_img, IMAGE_JPEG_QUALITY);
                    }
                    if ($image_type == "avif") {
                        imageavif($image, $new_full_img, IMAGE_JPEG_QUALITY);
                    }
                ...
                $image_info["fileMTime"] = filemtime(
                    GALLERY_ROOT . $image_dir . $image_file
                ); // also used for deleting data if time have changed
                $image_info["fileSize"] = sfpg_file_size(
                    filesize(GALLERY_ROOT . $image_dir . $image_file)
                );
                $image_info["imageSizeX"] = block_html(imagesx($image));
                $image_info["imageSizeY"] = block_html(imagesy($image));
                file_put_contents(
                    DATA_ROOT . "info/" . $image_dir . $image_file,
                    serialize($image_info)
                );
            }
            list($new_img_width, $new_img_height) = sfpg_aspect_resize(
                imagesx($image),
                imagesy($image),
                $max_width,
                $max_height,
            ...
            if (LOW_IMAGE_RESAMPLE_QUALITY) {
                if (THUMB_SQUARE) {
                    ...
                    imagecopyresized(
                        ...
                        0,
                        0,
                        round((imagesx($image) - $min_size) / 2),
                        0,
                        $new_img_width,
                        $new_img_height,
                        $min_size,
            ...
            if ($image_type == "avif") {
                imageavif($new_image, null, $jpeg_quality);
                imageavif($new_image, $image_path_file, $jpeg_quality);
            }
            if ($image_type == "svg") {
            ...
            if ($image_type == "webp") {
                imagewebp($new_image, null, $jpeg_quality);
...
function sfpg_dir_info($dir, $initial = true)
{
    ...
    if (
        DIR_THUMB_FROM_ICONS_DIR and
        file_exists(GALLERY_ROOT . ICONS_DIR . DIR_THUMB_FILE)
    ) {
    ...
    foreach (
        $dirs
        as $subdir // searching for dir-thumb in subdirs
    ) {
...
function sfpg_set_dir_info($dir)
{
    ...
    file_put_contents(
        DATA_ROOT . "info/" . $dir . "_sfpg_dir",
        serialize(sfpg_dir_info($dir))
    );
}
...
{
    $items = array_diff(scandir(DATA_ROOT . "info/" . $dir), [
        ".",
        "..",
        "_sfpg_dir",
    ]);
    foreach ($items as $item) {
        if (
            !is_dir(GALLERY_ROOT . $dir . $item) and
            !file_exists(GALLERY_ROOT . $dir . $item)
        ) {
...
function sfpg_javascript()
{
    ...
    echo "
		...
		function showMenu()
		{
			...
			{
				menu = '<span id=\"button_left\" class=\"sfpg_button\" onclick=\"cycleImg(-1)\">" .
        sts(TEXT_PREVIOUS) .
        "</span>';
				if (index)
				{
					menu += '<span id=\"button_close\" class=\"sfpg_button\" onclick=\"closeImageView()\">" .
        sts(TEXT_CLOSE_IMG_VIEW) .
        "</span>';
				}
				else
				{
					menu += '<span class=\"sfpg_button_disabled\">" .
        sts(TEXT_CLOSE_IMG_VIEW) .
        "</span>';
				}
				menu += '<span id=\"button_right\" class=\"sfpg_button\" onclick=\"cycleImg(1)\">" .
        sts(TEXT_NEXT) .
        "</span>';
				";
    if (TEXT_SLIDESHOW) {
        echo "
					if (slideshowActive)
					{
						menu += '<span class=\"sfpg_button_on\" onclick=\"slideshowActive=false; showMenu();\">" .
            sts(TEXT_SLIDESHOW) .
            "</span>';
					}
					else
					{
						menu += '<span class=\"sfpg_button\" onclick=\"slideshowActive=true; showMenu(); slideshow(true);\">" .
            sts(TEXT_SLIDESHOW) .
            "</span>';
					}";
    }
    ...
    if (TEXT_INFO) {
        echo "
				if (showInfo)
				{
					menu += '<span onclick=\"toggleInfo(showInfo);\" class=\"sfpg_button_on\">" .
            sts(TEXT_INFO) .
            "</span>';
				}
				else
				{
					menu += '<span onclick=\"toggleInfo(showInfo);\" class=\"sfpg_button\">" .
            sts(TEXT_INFO) .
            "</span>';
				}";
    }
    echo "
			if (index)
			{
				if (actualSize)
				{
					menu += '<span class=\"sfpg_button_on\" onclick=\"fullSize()\">" .
        sts(TEXT_ACTUAL_SIZE) .
        "</span>';
				}
				else
				{
					menu += '<span class=\"sfpg_button\" onclick=\"fullSize()\">" .
        sts(TEXT_ACTUAL_SIZE) .
        "</span>';
				}
			}
        ...
        "</span>';
			...
			";
    if (LINK_BACK) {
        echo "menu += '<span class=\"sfpg_button\" onclick=\"window.location=\'" .
            LINK_BACK .
            "\'\">" .
            sts(TEXT_LINK_BACK) .
            "</span>';
    ...
    if (ADMIN === true) {
        echo "
					if (selecting)
					{
						menu = '<span class=\"sfpg_button\" onclick=\"admDelete()\">" .
            sts(TEXT_DELETE) .
            "</span>';
						menu += '<span class=\"sfpg_button\" onclick=\"admRename()\">" .
            sts(TEXT_RENAME) .
            "</span>';
						menu += '<span class=\"sfpg_button\" onclick=\"admMove()\">" .
            sts(TEXT_MOVE) .
            "</span>';
						menu += '<span class=\"sfpg_button\" onclick=\"admMakeDir()\">" .
            sts(TEXT_MKDIR) .
            "</span>';
						menu += '<span class=\"sfpg_button\" onclick=\"admSetThumb()\">" .
            sts(TEXT_SET_DIR_THUMB) .
            "</span>';
						menu += '<span class=\"sfpg_button\" onclick=\"admUpload(true)\">" .
            sts(TEXT_UPLOAD) .
            "</span>';
						menu += '<span class=\"sfpg_button\" onclick=\"admDesc()\">" .
            sts(TEXT_DESCRIPTION) .
            "</span>';
						menu += '<span class=\"sfpg_button\" onclick=\"admSell()\">" .
            sts(TEXT_PAYPAL_FOR_SALE) .
            "</span>';
						menu += '<span class=\"sfpg_button\" onclick=\"invertSelection()\">" .
            sts(TEXT_INVERT_SELECTION) .
            "</span>';
						menu += '<span class=\"sfpg_button_on\" onclick=\"toggleSelect()\">" .
            sts(TEXT_ADMIN) .
            "</span>';
					}
					else
					{
						menu += '<span class=\"sfpg_button\" onclick=\"toggleSelect()\">" .
            sts(TEXT_ADMIN) .
            "</span>';
					}
				";
    ...
    echo "
        ...
        "';
					...
					info += '<strong>" .
        sts(TEXT_DIR_NAME) .
        "</strong><br><div class=\"sfpg_info_text\">'+dirInfo[id]['dirName'] + '</div><br>';";
    if (DIR_DESC_IN_INFO) {
        echo "
						if (isDef(dirInfo[id]['dirDesc']))
						{
							info += '<strong>" .
            sts(TEXT_DESCRIPTION) .
            "</strong><br><div class=\"sfpg_info_text\">'+dirInfo[id]['dirDesc']+'<br></div><br>';
						}";
    }
    if (TEXT_INFO_LABEL) {
        echo "
						info += '<strong>" .
            sts(TEXT_INFO_LABEL) .
            "</strong><br><div class=\"sfpg_info_text\">';
						info += '" .
            sts(TEXT_DATE) .
            ": '+dateFormat(dirInfo[id]['dirTime'])+'<br>';
						info += '" .
            sts(TEXT_DIRS) .
            ": '+dirInfo[id]['dirDirs']+'<br>';
						info += '" .
            sts(TEXT_IMAGES) .
            ": '+dirInfo[id]['dirImages']+'<br>';";
        if (SHOW_FILES) {
            echo "
							info += '" .
                sts(TEXT_FILES) .
                ": '+dirInfo[id]['dirFiles']+'<br>';";
        }
        echo "
						info += '</div><br>';
						";
    ...
    echo "
					info += '<strong>" .
        sts(TEXT_LINKS) .
        "</strong><br><a href=\"'+phpSelf+'?sfpg='+dirInfo[id]['dirLink']+'\">" .
        sts(TEXT_DIRECT_LINK_GALLERY) .
        "</a><br>';";
    if (ZIP_ENABLE) {
        echo "
							info += '<br>" .
            sts(TEXT_ZIP_DL) .
            "<br><br>';
							info += '<span id=\"zipspan\"><form method=\"post\" id=\"zipform\"><input type=\"password\" id=\"password\" name=\"password\"><input type=\"hidden\" name=\"zipdl\" value=\"x\"></form><span class=\"sfpg_button\" onclick=\"generateZip()\">" .
            sts(TEXT_ZIP_BUTTON) .
            "</span></span><br><br>';"; // the password input is to make sure that web crawlers do not post the form
    }
    echo "
				}
				else if (type == 'img')
				{
					...
					info += '<strong>" .
        sts(TEXT_IMAGE_NAME) .
        "</strong><br><div class=\"sfpg_info_text\">'+imgInfo[id]['imageName'] + '</div><br>';
					";
    if (PAYPAL_ENABLED) {
        echo "
            ...
            "</strong><br><div class=\"sfpg_info_text\">';
							info += '" .
            sts(TEXT_PAYPAL_PRICE) .
            " (" .
            sts(PAYPAL_CURRENCY) .
            "): '+((isDef(imgInfo[id]['sellPrice']))?imgInfo[id]['sellPrice']:'')+'<br>';
							info += '" .
            sts(TEXT_PAYPAL_STATUS) .
            ": '+(((isDef(imgInfo[id]['sellStatus']))&&(imgInfo[id]['sellStatus']==1))?'" .
            sts(TEXT_PAYPAL_AVAILABLE) .
            "':'" .
            sts(TEXT_PAYPAL_SOLD) .
            "')+'<br>';
							if (isDef(imgInfo[id]['sellID']))
							{
								info += '" .
            sts(TEXT_PAYPAL_ITEM_ID) .
            ": '+imgInfo[id]['sellID']+'<br>';
							}
							if ((isDef(imgInfo[id]['sellStatus']))&&(imgInfo[id]['sellStatus']==1))
							{
								info += '<br><span onclick=\"window.location=\''+phpSelf+'?cmd=buy&sfpg='+imgInfo[id]['imageLink']+'\'\" class=\"sfpg_button\">" .
            sts(TEXT_PAYPAL_BUY) .
            "</span><br><br>';
							}
							else
							{
								info += '<br><span class=\"sfpg_button_disabled\">" .
            sts(TEXT_PAYPAL_BUY) .
            "</span><br><br>';
							}
							info += '</div><br>';
    ...
    echo "
					if (!isDef(imgInfo[id]['firstView']))
					{
						if (isDef(imgInfo[id]['fileDescription']))
						{
							info += '<strong>" .
        sts(TEXT_DESCRIPTION) .
        "</strong><br><div class=\"sfpg_info_text\">'+imgInfo[id]['fileDescription']+'<br></div><br>';
						}";
    if (TEXT_INFO_LABEL) {
        echo "
							info += '<strong>" .
            ...
            "</strong><br><div class=\"sfpg_info_text\">';
							info += '" .
            sts(TEXT_DATE) .
            ": '+dateFormat(imgInfo[id]['fileMTime'])+'<br>';
							info += '" .
            sts(TEXT_IMAGESIZE) .
            ": '+imgInfo[id]['imageSizeX']+' x '+imgInfo[id]['imageSizeY']+'<br>';
							info += '" .
            sts(TEXT_SCALED_TO) .
            ": <span id=\"img_resize\"></span><br>';
							info += '" .
            sts(TEXT_FILESIZE) .
            ": '+imgInfo[id]['fileSize']+'<br>';
							info += '" .
            sts(TEXT_IMAGE_NUMBER) .
            ": '+id+' / '+(imgInfo.length-1)+'<br>';
							info += '</div><br>';";
    }
    if (SHOW_EXIF_INFO) {
        echo "
							var exifInfo = (isDef(imgInfo[id]['exifDate'])?'" .
            sts(TEXT_EXIF_DATE) .
            ": '+dateFormat(imgInfo[id]['exifDate'])+'<br>':'');
							exifInfo += (isDef(imgInfo[id]['exifModel'])?'" .
            sts(TEXT_EXIF_CAMERA) .
            ": '+imgInfo[id]['exifModel']+'<br>':'');
							exifInfo += (isDef(imgInfo[id]['exifISOSpeedRatings'])?'" .
            sts(TEXT_EXIF_ISO) .
            ": '+imgInfo[id]['exifISOSpeedRatings']+'<br>':'');
							exifInfo += (isDef(imgInfo[id]['exifExposureTime'])?'" .
            sts(TEXT_EXIF_SHUTTER) .
            ": '+imgInfo[id]['exifExposureTime']+'<br>':'');
							exifInfo += (isDef(imgInfo[id]['exifFNumber'])?'" .
            sts(TEXT_EXIF_APERTURE) .
            ": '+imgInfo[id]['exifFNumber']+'<br>':'');
							exifInfo += (isDef(imgInfo[id]['exifFocalLength'])?'" .
            sts(TEXT_EXIF_FOCAL) .
            ": '+imgInfo[id]['exifFocalLength']+'<br>':'');
							exifInfo += (isDef(imgInfo[id]['exifFlash'])?'" .
            sts(TEXT_EXIF_FLASH) .
            ": '+imgInfo[id]['exifFlash']+'<br>':'');
							exifInfo += (isDef(imgInfo[id]['exifGPSLatitude'])?'" .
            sts(TEXT_EXIF_LATITUDE) .
            ": '+imgInfo[id]['exifGPSLatitude']+'<br>':'');
							exifInfo += (isDef(imgInfo[id]['exifGPSLongitude'])?'" .
            sts(TEXT_EXIF_LONGITUDE) .
            ": '+imgInfo[id]['exifGPSLongitude']+'<br>':'');
							";
        if (TEXT_EXIF_MAP and EXIF_MAP_LINK) {
            echo "
								if (isDef(imgInfo[id]['exifGPSLatitudeDec']) && isDef(imgInfo[id]['exifGPSLongitudeDec']))
								{
									tmpStr = '<a target=\"_blank\" href=\"" .
                sts(EXIF_MAP_LINK) .
                "\">" .
                sts(TEXT_EXIF_MAP) .
                "</a>';
        ...
        echo "
							if (exifInfo != '')
							{
								info += '<strong>" .
            sts(TEXT_EXIF) .
            "</strong><br><div class=\"sfpg_info_text\">'+exifInfo+'</div><br>';
							}";
    }
    if (TEXT_EXIF_MAP_EMBED and EXIF_MAP_EMBED_LINK) {
        echo "
							if (isDef(imgInfo[id]['exifGPSLatitudeDec']) && isDef(imgInfo[id]['exifGPSLongitudeDec']))
							{
								tmpStr = '<iframe class=\"map_iframe\" src=\"" .
            sts(EXIF_MAP_EMBED_LINK) .
            "\"></iframe>';
								tmpStr = tmpStr.replace('[lat]', imgInfo[id]['exifGPSLatitudeDec']);
								tmpStr = tmpStr.replace('[long]', imgInfo[id]['exifGPSLongitudeDec']);
								info += '<strong>" .
            sts(TEXT_EXIF_MAP_EMBED) .
            "</strong><br>'+tmpStr+'<br><br>';
							}
							";
    }
    if (SHOW_IPTC_INFO) {
        echo "var iptcInfo = '';
							";
        foreach (IPTC as $iptcID => $iptcText) {
            echo "iptcInfo += (isDef(imgInfo[id]['" .
                $iptcID .
                "'])?'" .
                sts($iptcText) .
                ": '+imgInfo[id]['" .
        ...
        echo "
							if (iptcInfo != '')
							{
								info += '<strong>" .
            sts(TEXT_IPTC) .
            "</strong><br><div class=\"sfpg_info_text\">'+iptcInfo+'</div><br>';
							}";
    }
    if (PNG_TEXT_CHUNKS) {
        echo "
    ...
    echo "
					...
					else
					{
						info += '<br><strong>" .
        sts(TEXT_FIRST_VIEW) .
        "</strong><br><br><span id=\"img_size\"></span><br><br>';
					}
					info += '<strong>" .
        sts(TEXT_LINKS) .
        "</strong><br>';
					info += '<a href=\"'+phpSelf+'?sfpg='+imgInfo[id]['imageLink']+'\">" .
        sts(TEXT_DIRECT_LINK_IMAGE) .
        "</a><br>';
					" .
        (TEXT_DOWNLOAD
            ? "info += '<a href=\"'+phpSelf+'?cmd=dl&sfpg='+imgInfo[id]['imageLink']+'\">" .
                sts(TEXT_DOWNLOAD) .
                "</a><br><br>';"
            : "") .
        "
				...
				else if (type == 'file')
				{
					...
					{
						info += '<br><br>" .
        sts(TEXT_NO_THUMB_FOR_FILE) .
        "<br>';
					}
					info += '</div>';
					info += '<strong>" .
        sts(TEXT_FILE_NAME) .
        "</strong><br><div class=\"sfpg_info_text\">'+fileInfo[id]['fileName']+'</div><br>';
					if (isDef(fileInfo[id]['description']))
					{
						info += '<strong>" .
        sts(TEXT_DESCRIPTION) .
        "</strong><br><div class=\"sfpg_info_text\">'+fileInfo[id]['description']+'<br></div><br>';
					}";
    if (TEXT_INFO_LABEL) {
        echo "
						info += '<strong>" .
            ...
            "</strong><br><div class=\"sfpg_info_text\">';
						info += '" .
            sts(TEXT_DATE) .
            ": '+dateFormat(fileInfo[id]['fileTime'])+'<br>';
						info += '" .
            sts(TEXT_FILESIZE) .
            ": '+fileInfo[id]['fileSize']+'<br>';
						info += '</div><br>';";
    }
    echo "
				}
		...
		function toggleInfo(status)
		{
			...
			else
			{
				...
				gebi('box_image').style.left='" .
        INFO_BOX_WIDTH .
        "px';
				gebi('box_gallery').style.left='" .
        INFO_BOX_WIDTH .
        "px';
				gebi('box_wait').style.left='" .
        INFO_BOX_WIDTH .
        "px';
			}
			showInfo = !status;
		...
		function showImage(stage)
		{
			...
			gebi('wait').innerHTML = '<div class=\"loading\">" .
        sts(TEXT_IMAGE_LOADING) .
        "' + waitSpin[waitSpinNr] + '</div>';
			waitSpinNr++;
			if ((stage<3) && index)
			{
    ...
    if (ADMIN === true) {
		...
		function admSetThumb()
		{
			...
			{
				if (confirm('" .
            sts(TEXT_SETTING_THUMB) .
            "' + imgInfo[selectedImages[0]]['imageName']) == true)
				{
					sendForm=true;
				}
			}
		...
		function admSell()
		{
            ...
            sts(TEXT_PAYPAL_FOR_SALE) .
            "</strong>: '+eName+'<br><br>'+
				'<span class=\"sfpg_button\" onclick=\"admSellPost(\''+eLink+'\', \''+eName+'\', \'ok\')\">" .
            sts(TEXT_OK) .
            "</span>'+
				'<span class=\"sfpg_button\" onclick=\"admSellPost(\''+eLink+'\', \''+eName+'\',\'del\')\">" .
            sts(TEXT_DELETE) .
            "</span>'+
				'<span class=\"sfpg_button\" onclick=\"admSellPost(0,0,\'cancel\')\">" .
            sts(TEXT_CANCEL) .
            "</span><br><br>'+
				'" .
            sts(TEXT_PAYPAL_PRICE) .
            " (" .
            sts(PAYPAL_CURRENCY) .
            "): <input type=\"text\" value=\"'+ePrice+'\" id=\"ePrice\"><br><br>'+
				'" .
            sts(TEXT_PAYPAL_STATUS) .
            "<br>'+
				'<input type=\"radio\" id=\"eStatus1\" name=\"eStatus\" value=\"x\"'+(eStatus==1?' checked':'')+'><label for=\"eStatus1\">" .
            sts(TEXT_PAYPAL_AVAILABLE) .
            "</label><br>'+
				'<input type=\"radio\" id=\"eStatus2\" name=\"eStatus\" value=\"x\"'+(eStatus!=1?' checked':'')+'><label for=\"eStatus2\">" .
            sts(TEXT_PAYPAL_SOLD) .
            "</label><br><br>'+
				'" .
            sts(TEXT_PAYPAL_ITEM_ID) .
            ": <input type=\"text\" value=\"'+eIdent+'\" id=\"eIdent\">';
				var ele=gebi('box_admin');
		...
		function admSellPost(link, name, action)
		{
			...
			{
				if (confirm('" .
            sts(TEXT_DELETE . " " . TEXT_PAYPAL_FOR_SALE) .
            ": '+name+'?') == true)
				{
					var form = makeForm();
		...
		function admDesc()
		{
            ...
            sts(TEXT_DESCRIPTION) .
            ": '+eName+'<br><br>'+
				'<span class=\"sfpg_button\" onclick=\"admDescPost(\''+eLink+'\', \''+eName+'\', \'ok\')\">" .
            sts(TEXT_OK) .
            "</span>'+
				'<span class=\"sfpg_button\" onclick=\"admDescPost(\''+eLink+'\', \''+eName+'\',\'del\')\">" .
            sts(TEXT_DELETE) .
            "</span>'+
				'<span class=\"sfpg_button\" onclick=\"admDescPost(0,0,\'cancel\')\">" .
            sts(TEXT_CANCEL) .
            "</span><br><br>'+
		...
		function admUpload(show)
		{
            ...
            sts(TEXT_UPLOAD) .
            "\"> '+
				'<input type=\"button\" onclick=\"admUpload(false)\" value=\"" .
            sts(TEXT_CANCEL) .
            "\">'+
		...
		function admDescPost(link, name, action)
		{
			...
			{
				if (confirm('" .
            sts(TEXT_DELETE . " " . TEXT_DESCRIPTION) .
            ": '+name+'?') == true)
				{
					var form = makeForm();
    ...
    echo "
        ...
        (THUMB_CHARS_MAX
            ? "content += '" .
                TEXT_DIR_MARK_START .
                "'+thumbDisplayName(dirInfo[elementNumber]['dirName'])+'" .
                TEXT_DIR_MARK_END .
                "';"
            : "") .
        "
        ...
        (DIR_DESC_IN_GALLERY ? "true" : "false") .
        " && (type == 'desc'))
			{
    ...
    if (PAYPAL_ENABLED) {
        echo "
            ...
            "';
				...
				gebi('paypalCancelReturn').value = SelfUrl+'?sfpg=" .
            (sfpg_url_decode(@$_GET["sfpg"]) != false ? $_GET["sfpg"] : "") .
            "&info=1';
    ...
    if (GALLERY and is_array($links)) {
        for ($i = 0; $i < count($links); $i++) {
            if ($links[$i] !== "") {
                $gal_dirs .= $links[$i] . "/";
                $display_name = in_array(DIR_NAME_FILE, $misc)
                    ? @file(GALLERY_ROOT . $gal_dirs . DIR_NAME_FILE)
                    : "";
                if ($display_name) {
        ...
        $current_dir_name =
            count($a_links) == 0 ? TEXT_HOME : $a_names[count($a_links) - 1];
    } else {
    ...
    if (
        DIR_THUMB_FILE and
        file_exists(GALLERY_ROOT . GALLERY . DIR_THUMB_FILE) and
        file_exists(DATA_ROOT . "info/" . GALLERY . DIR_THUMB_FILE)
    ) {
        $filed = unserialize(
            file_get_contents(DATA_ROOT . "info/" . GALLERY . DIR_THUMB_FILE)
        );
        if (
            filemtime(GALLERY_ROOT . GALLERY . DIR_THUMB_FILE) !=
            $filed["fileMTime"]
        ) {
    ...
    foreach ($dirs as $val) {
        ...
        $filed = unserialize(
            file_get_contents(
                DATA_ROOT . "info/" . GALLERY . $val . "/_sfpg_dir"
            )
        );
        if (
            DELETE_EMPTY_DIRS and
            $filed["dirDirs"] == 0 and
            $filed["dirImages"] == 0 and
            $filed["dirFiles"] == 0
        ) {
        ...
        if (
            ...
            $display_name = @file(
                GALLERY_ROOT . GALLERY . $val . "/" . DIR_NAME_FILE
            );
            if ($display_name) {
                $display_name = trim($display_name[0]);
                $display_name = clean_html($display_name);
            ...
            $desc = @file_get_contents(
                GALLERY_ROOT . GALLERY . $val . "/" . DIR_DESC_FILE
            );
            if ($desc) {
    ...
    $filed = unserialize(
        file_get_contents(DATA_ROOT . "info/" . GALLERY . "_sfpg_dir")
    );
    if (
        count($dirs) !== $filed["dirDirs"] or
        count($images) !== $filed["dirImages"] or
        count($files) !== $filed["dirFiles"]
    ) {
        sfpg_set_dir_info(GALLERY);
        $filed = unserialize(
            file_get_contents(DATA_ROOT . "info/" . GALLERY . "_sfpg_dir")
    ...
    $dir_info["dirImages"] = $filed["dirImages"];
    $dir_info["dirFiles"] = $filed["dirFiles"];
    $desc = in_array(DIR_DESC_FILE, $misc)
        ? @file_get_contents(GALLERY_ROOT . GALLERY . DIR_DESC_FILE)
        : "";
    if ($desc) {
        $dir_info["dirDesc"] = clean_html($desc);
    }
    ...
    foreach ($images as $val) {
        ...
        if (SHOW_IMAGE_DAYS) {
            if (
                filemtime(GALLERY_ROOT . GALLERY . $val) <
                time() - SHOW_IMAGE_DAYS * 86400
            ) {
                $showImage = false;
            }
        }
        if ($showImage) {
            ...
            if (file_exists(DATA_ROOT . "info/" . GALLERY . $val)) {
                $image_info = unserialize(
                    file_get_contents(DATA_ROOT . "info/" . GALLERY . $val)
                );
                if (
                    filemtime(GALLERY_ROOT . GALLERY . $val) !=
                    $image_info["fileMTime"]
                ) {
                    // if image time have changed, delete data to have new thumb created
                    sfpg_delete_from_data_root(GALLERY . $val);
                    $image_info["firstView"] = 1;
                } else {
                    if (in_array($val . DESC_EXT, $misc)) {
                        // if description file exists, add to array
                        $desc = @file_get_contents(
                            GALLERY_ROOT . GALLERY . $val . DESC_EXT
                        );
            ...
            if (PAYPAL_ENABLED) {
                $sell = in_array($val . PAYPAL_EXTENSION, $misc)
                    ? @file(
                        GALLERY_ROOT . GALLERY . $val . PAYPAL_EXTENSION,
    ...
    if (count($files)) {
        ...
        foreach ($files as $val) {
            ...
            if (
                FILE_THUMB_EXT and
                file_exists(GALLERY_ROOT . GALLERY . $val . FILE_THUMB_EXT)
            ) {
                $file_info["fileThumb"] = sfpg_url_string(
                    GALLERY,
                    $val . FILE_THUMB_EXT
                );
            ...
            $file_info["fileSize"] = sfpg_file_size(
                @filesize(GALLERY_ROOT . GALLERY . $val)
            );
            if (in_array($val . DESC_EXT, $misc)) {
                $desc = @file_get_contents(
                    GALLERY_ROOT . GALLERY . $val . DESC_EXT
...
if (!defined("SECURITY_PHRASE")) {
    ...
    @include DATA_ROOT . "sp.php";
    if (!defined("SECURITY_PHRASE")) {
        echo '<br>PHP do not have access to create files in the defined DATA_ROOT ("' .
            DATA_ROOT .
            '").<br>See readme.txt for DATA_ROOT description.';
        exit();
    }
}
if (SECURITY_PHRASE == "") {
    echo '<br>The SECURITY_PHRASE in "' .
        DATA_ROOT .
        'sp.php" cannot be empty.<br>See readme.txt for DATA_ROOT description.';
    exit();
}
...
if (PASSWORD !== "") {
    ...
    if (
        ($_SESSION["sfpg_access"] ?? false) !== true or
        ...
        if (isset($_POST["pw"]) and $_POST["pw"] === PASSWORD) {
            ...
            echo '<!DOCTYPE html><html lang="' .
                HTML_LANGUAGE .
                '"><head><meta name="viewport" content="width=device-width, initial-scale=1"><meta charset="' .
                CHARSET .
                ...
                '<input type="submit" name="su" value="' .
                TEXT_LOGIN .
                '">' .
                "</form></body></html>";
            session_write_close();
...
if (ZIP_ENABLE and isset($_POST["zipdl"])) {
    ...
    if (count($files_for_zip) > 0) {
        ...
        if (
            file_exists($zip_dir . $zip_name) and
            file_exists($zip_dir . $zip_name . ".hash") and
            $zip_hash === file_get_contents($zip_dir . $zip_name . ".hash")
        ) {
        ...
        foreach ($files_for_zip as $file) {
            $zip->addFile(
                realpath(GALLERY_ROOT . $file),
                substr($file, strlen(GALLERY))
            );
            if (ZIP_COMPRESSION) {
...
if (isset($_GET["cmd"]) and $get_set) {
    ...
    if (SHOW_FILES and $_GET["cmd"] == "file") {
        if (preg_match("#^(/|([A-Z]:)?(\\\\|/))#i", GALLERY_ROOT)) {
            // if GALLERY_ROOT is an absolute path
...
if (ADMIN === true) {
    if (isset($_POST["func"])) {
        if ($_POST["func"] === "move") {
            if (
                isset($_POST["toFolder"]) and
                isset($_POST["elems"]) and
                is_array($_POST["elems"]) and
                count($_POST["elems"]) > 0
            ) {
                $to_dir_array = sfpg_url_decode($_POST["toFolder"]);
                if (
                    $to_dir_array !== false and
                    $to_dir_array[1] === "" and
                    is_dir(GALLERY_ROOT . $to_dir_array[0])
                ) {
                    ...
                    foreach ($_POST["elems"] as $elem) {
                        ...
                        if ($move_elem) {
                            if (
                                $move_elem[1] === "" and
                                is_dir(GALLERY_ROOT . $move_elem[0])
                            ) {
                                ...
                                if (file_exists($file_to_move . DESC_EXT)) {
                                    rename(
                                        $file_to_move . DESC_EXT,
                                        $to_dir . $move_elem[1] . DESC_EXT
                                    );
                                }
                                if (
                                    sfpg_image_type($file_to_move) and
                                    file_exists(
                                        $file_to_move . PAYPAL_EXTENSION
                                    )
                                ) {
                                    rename(
                                        $file_to_move . PAYPAL_EXTENSION,
                                    ...
                                    file_exists($file_to_move . FILE_THUMB_EXT)
                                ) {
                                    rename(
                                        $file_to_move . FILE_THUMB_EXT,
                                        $to_dir . $move_elem[1] . FILE_THUMB_EXT
                                    );
                                }
                            }
                        }
        ...
        if ($_POST["func"] === "setthumb") {
            if (isset($_POST["elems"])) {
                if (count($_POST["elems"]) == 1) {
                    ...
                    if ($new_thumb) {
                        if (
                            sfpg_image_type($new_thumb[1]) ===
                            sfpg_image_type(DIR_THUMB_FILE)
                        ) {
                            copy(
                                GALLERY_ROOT . $new_thumb[0] . $new_thumb[1],
                                GALLERY_ROOT . $new_thumb[0] . DIR_THUMB_FILE
                            );
                            sfpg_set_dir_info($new_thumb[0]);
                        } else {
                            echo TEXT_WRONG_FILETYPE .
                                sfpg_image_type(DIR_THUMB_FILE);
                            exit();
                        }
                    }
        ...
        if (
            $_POST["func"] === "rename" and
            isset($_POST["eleToRen"]) and
            isset($_POST["newName"])
        ) {
            ...
            if ($newName !== "" and $eleToRen) {
                ...
                if ($eleToRen) {
                    if ($eleToRen[1] !== "") {
                        $newName .=
                            (!SHOW_IMAGE_EXT and sfpg_image_type($eleToRen[1]))
                                ? sfpg_ext($eleToRen[1])
                                : "";
                        if (
                            !file_exists(
                                GALLERY_ROOT . $eleToRen[0] . $newName
                            ) and
                            !is_dir(GALLERY_ROOT . $eleToRen[0] . $newName)
                        ) {
                            ...
                            rename(
                                GALLERY_ROOT . $eleToRen[0] . $eleToRen[1],
                                GALLERY_ROOT . $eleToRen[0] . $newName
                            );
                        }
                    } else {
                        ...
                        $newPath =
                            substr($oldPath, 0, strrpos($oldPath, "/") + 1) .
                            $newName;
        ...
        if ($_POST["func"] === "sell") {
            ...
            if ($action === "del") {
                ...
                if ($eleWd) {
                    sfpg_delete(
                        GALLERY_ROOT . $eleWd[0] . $eleWd[1] . PAYPAL_EXTENSION
                    );
                }
            } elseif ($action === "save") {
                $eleWd = sfpg_url_decode($ele);
...
echo '<!DOCTYPE html><html lang="' .
    ...
    '"><head><meta name="viewport" content="width=device-width, initial-scale=1"><meta charset="' .
    CHARSET .
    '"><meta name="description" content="Single File PHP Gallery"><title>' .
    TEXT_GALLERY_NAME .
    "</title><style>" .
    '
	img
	...
	.sfpg_button_disabled
	{
		cursor:default;
		border:1px solid ' .
    COLOR_BUTTON_BORDER_OFF .
    ';
		background:' .
    COLOR_BUTTON_BACK_OFF .
    ';
		color:' .
    COLOR_BUTTON_TEXT_OFF .
    ';
	}
	...
	{
		border:1px solid ' .
    COLOR_BUTTON_NAV_BORDER .
    ';
		background:' .
    COLOR_BUTTON_NAV_BACK .
    ';
		color:' .
    COLOR_BUTTON_NAV_TEXT .
    ';
	}
	...
	.thumbbox, .descbox
	{
		...
		*display:inline;
		width:' .
    (2 * (THUMB_BORDER_WIDTH + THUMB_MARGIN + THUMB_BOX_MARGIN) +
        THUMB_MAX_WIDTH +
        2) .
    'px;
		height:' .
	...
	{
		width:' .
    (2 * (THUMB_BORDER_WIDTH + THUMB_MARGIN) + THUMB_MAX_WIDTH) .
    'px;
		height:' .
    (THUMB_BORDER_WIDTH * 2 + THUMB_MARGIN + THUMB_MAX_HEIGHT + 6) .
    'px;
		margin:0px; 
		padding:0px;
	}
    ...
    'px;
		padding:0px;
		width:' .
    (2 * (THUMB_BORDER_WIDTH + THUMB_MARGIN) + THUMB_MAX_WIDTH + 2) .
    'px;
		height:' .
	...
	.full_image, .full_image_no_resize
	{
		cursor:pointer;
		border:' .
    FULLIMG_BORDER_WIDTH .
    "px solid " .
    COLOR_FULLIMG_BORDER .
    ';
		width: auto;
		height : auto;
		max-height: 100%;
    ...
    'px;
		bottom:' .
    (MENU_BOX_HEIGHT + THUMB_BOX_MARGIN) .
    'px;
		left:' .
    THUMB_BOX_MARGIN .
    'px;
		width:' .
    (INFO_BOX_WIDTH - 2 * THUMB_BOX_MARGIN) .
    'px;
		z-index:1040;
		visibility:hidden;
		overflow:auto;
		border:1px solid ' .
    COLOR_INFOBOX_BORDER .
    ';
		' .
    (ROUND_CORNERS ? "border-radius:" . ROUND_CORNERS * 2 . "px;" : "") .
    '
...
if (PAYPAL_ENABLED and $get_set and @$_GET["cmd"] == "buy" and IMAGE != "") {
    $sell = @file(
        GALLERY_ROOT . GALLERY . IMAGE . PAYPAL_EXTENSION,
        FILE_IGNORE_NEW_LINES
    );
    if ($sell == false or $sell[1] != 1) {
        echo "<body><br>" . TEXT_PAYPAL_OUT_BACK . "</body>";
    } else {
        echo '<body onload="paypal(' .
            IMAGE_ID_IN_URL .
            ');">' .
            "<br>" .
            ...
            '<input type="hidden" id="paypalItemNumber" name="item_number" value="' .
            GALLERY .
            IMAGE .
            '">' .
